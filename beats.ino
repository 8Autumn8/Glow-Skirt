...
byte binFloor[8] = {117, 103, 40, 39, 38,  36,  22, 18}; // floor for subtraction -> higher frequencies
...
void runFHT() {
  ...
  for (int i = 0; i < LOG_N; i++) {
    ...
    //get the raw values before supression 
    float magnitude = x_scaled * x_scaled;
  ...
  Serial.print(totalMagnitude);
  Serial.print('\t');
  if (totalMagnitude > beatThreshold){
    isBeat = 2;
  }
  Serial.print(0);
  Serial.print('\t');
